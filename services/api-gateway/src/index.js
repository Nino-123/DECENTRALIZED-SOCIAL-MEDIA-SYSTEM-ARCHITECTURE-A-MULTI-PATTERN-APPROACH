...
app.post("/posts", authenticateToken, async (req, res) => {
  ...
  try {
    const newPostQuery =
      "INSERT INTO posts(content, user_id) VALUES($1, $2) RETURNING id, content, created_at";
    ...
    if (federationServiceUrl) {
      ...
      axios
        .post(`${federationServiceUrl}/internal/federate`, {
          post_id: newPost.id,
          username: username,
...
app.get("/posts", async (req, res) => {
  try {
    ...
    const localPosts = localPostsResult.rows.map((post) => ({
      ...post,
      isFederated: false,
    }));
    ...
    const federatedPosts = federatedPostsResult.rows.map((post) => ({
      ...post,
      isFederated: true,
    }));
...
app.delete("/posts/:id", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (federationServiceUrl) {
      ...
      axios
        .post(`${federationServiceUrl}/internal/federate-delete`, {
          post_id: postId,
        })
        .catch((err) => {
          console.error(
            "Failed to notify federation service of deletion:",
            err.message
          );
        });
    }
...
app.get('/profile/:username', async (req, res) => {
    ...
    const { instance } = req.query; // e.g., /profile/user1?instance=some-other-blog.com

    try {
        let userPosts;
        let profileData = { username, instance: instance || 'local' };

        if (instance) {
            // This is a federated user
            const federatedQuery = `
                SELECT id, content, created_at, original_user_username as username, original_instance_hostname as instance, 'federated' as type
                FROM federated_posts
                WHERE original_user_username = $1 AND original_instance_hostname = $2
                ORDER BY created_at DESC;
            `;
            const result = await pool.query(federatedQuery, [username, instance]);
            userPosts = result.rows.map(post => ({ ...post, isFederated: true }));
        } else {
            ...
            `;
            const result = await pool.query(localQuery, [username]);
            userPosts = result.rows.map(post => ({ ...post, isFederated: false }));
        }
        
        res.status(200).json({ profile: profileData, posts: userPosts });

    } catch (err) {
        console.error(`Error fetching profile for ${username}@${instance || 'local'}:`, err);
        res.status(500).json({ error: 'Internal server error' });
    }
});
...
app.get("/admin/peers", authenticateToken, async (req, res) => {
  try {
    ...
    console.error(
      "Error fetching peers from federation service:",
      error.message
    );
...
app.post("/admin/peers", authenticateToken, async (req, res) => {
  ...
  try {
    const federationServiceUrl = process.env.FEDERATION_SERVICE_URL;
    const response = await axios.post(`${federationServiceUrl}/peers`, {
      hostname,
    });
...
app.delete("/admin/peers", authenticateToken, async (req, res) => {
  ...
  try {
    const federationServiceUrl = process.env.FEDERATION_SERVICE_URL;
    const response = await axios.delete(`${federationServiceUrl}/peers`, {
      data: { hostname },
    });
    res.status(response.status).json(response.data);
  } catch (error) {
